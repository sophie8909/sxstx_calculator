...
export function renderTargetLevels(container) {
  ...
  const relicSection = detailed.lastElementChild; const relicModeWrap = el('div', ['flex', 'justify-center', 'mb-2']); const relicModeSelect = el('select', ['hidden']); relicModeSelect.id = 'target-relic-layout-mode'; [['element', 'target_relic_mode_element'], ['individual', 'target_relic_mode_individual']].forEach(([value, key]) => { const option = el('option'); option.value = value; option.textContent = t(key); relicModeSelect.appendChild(option); }); relicModeSelect.value = saved['target-relic-layout-mode'] || 'element'; relicModeWrap.appendChild(relicModeSelect); const modeButtons = el('div', ['segmented-control']); [['element', 'target_relic_mode_element'], ['individual', 'target_relic_mode_individual']].forEach(([value, key]) => { const button = el('button', ['target-relic-mode-btn', 'rounded-full', 'px-3', 'py-1', 'text-sm', 'font-semibold']); button.type = 'button'; button.dataset.mode = value; button.textContent = t(key); button.setAttribute('aria-pressed', String(relicModeSelect.value === value)); modeButtons.appendChild(button); }); relicModeWrap.appendChild(modeButtons); relicSection.appendChild(relicModeWrap);
  const relicElementGrid = el('div', ['target-relic-elements', 'grid', 'grid-cols-2', 'md:grid-cols-5', 'gap-3']); ELEMENT_TARGET_IDS.forEach((id, index) => { const group = makeScoredGroup(id, t('target_element_resonance', { index: index + 1 })); group.querySelector('[data-target-score-for]').dataset.targetScoreFor = 'relic-element-' + (index + 1); relicElementGrid.appendChild(group); }); relicSection.appendChild(relicElementGrid);
  const relicIndividualGrid = el('div', ['target-relic-individuals', 'hidden', 'grid', 'grid-cols-2', 'md:grid-cols-4', 'xl:grid-cols-5', 'gap-3']); RELIC_TARGET_IDS.forEach((id, index) => relicIndividualGrid.appendChild(makeScoredGroup(id, t('target_relic_number', { index: index + 1 })))); relicSection.appendChild(relicIndividualGrid);
...
export function renderCharBed(container) {
  ...
  const row = ({ id, type = 'number', label, placeholder = '', readOnly = false, min, step }) => {
    ...
    const line = el('div', ['flex', 'flex-col', 'items-start', 'gap-2']);
    ...
    const right = el('div', ['flex', 'items-center', 'justify-end', 'w-full']);
  ...
  const expRequiredButton = el('button', ['hidden', 'btn-secondary', 'w-full', 'md:w-auto']);
