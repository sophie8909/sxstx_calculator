...
import { getGoogleSheetCsvUrl, loadGameSettings, loadUpgradeCostTablesForSeason, clearDataServiceMemoryCache } from '../../services/dataService.js';
...
export let SPEEDUP_HOURS_PER_USE = 0;
export let NEXT_SEASON_EXP_HOARD_HOURS = 0;
export let NEXT_SEASON_EXP_HOARD_REMINDER_HOURS = 0;
export let RELIC_COUNT = 0;
...
export let STAMINA_BIG_MINE_RATE = 0;
export let STAMINA_BIG_MINE_EXPECTED_MULTIPLIER = 1;
...
export function applyGameSettings(rows) {
  const settings = Object.fromEntries((rows || []).map((row) => [String(row.key || '').trim(), Number(row.value)]));
  SPEEDUP_HOURS_PER_USE = settings.speedup_hours_per_use;
  NEXT_SEASON_EXP_HOARD_HOURS = settings.next_season_exp_hoard_hours;
  NEXT_SEASON_EXP_HOARD_REMINDER_HOURS = NEXT_SEASON_EXP_HOARD_HOURS - SPEEDUP_HOURS_PER_USE;
  RELIC_COUNT = settings.relic_count;
  STAMINA_BIG_MINE_RATE = settings.stamina_big_mine_rate;
  STAMINA_BIG_MINE_EXPECTED_MULTIPLIER = 1 + STAMINA_BIG_MINE_RATE;
  MATERIAL_DAILY_DEFAULTS.bond.freeze_dried = settings.bond_freeze_dried_daily;
  const required = [SPEEDUP_HOURS_PER_USE, NEXT_SEASON_EXP_HOARD_HOURS, RELIC_COUNT, STAMINA_BIG_MINE_RATE, MATERIAL_DAILY_DEFAULTS.bond.freeze_dried];
  if (required.some((value) => !Number.isFinite(value))) throw new Error('Google game settings are incomplete.');
}
...
export async function loadDataForSeason(seasonId) {
  ...
  let loaded;
  state.missingFiles = [];
  applyGameSettings(await loadGameSettings());

  try {
    loaded = await loadUpgradeCostTablesForSeason(targetSeason);
  } catch (err) {
    console.warn('[data load] Google upgrade-cost data is unavailable', err);
    loaded = {};
  ...
  for (const [key, gid] of Object.entries(REMOTE_DATA_FILES_CONFIG)) {
    ...
    try {
      ...
      console.warn(`[data load] Google data is unavailable for ${key} (${url})`, err);
